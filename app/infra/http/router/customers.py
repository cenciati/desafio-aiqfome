...
from app.__core__.application.presenters.customer_presenter import \
    CustomerPresenter
...
from app.infra.dependency import (get_delete_customer_use_case,
...
async def list_customers(
    ...
    try:
        ...
        output_dto = await list_customers_use_case.execute(input_dto)
        return {
            "data": CustomerPresenter.present(output_dto.data),
            "pagination": output_dto.pagination,
        }
...
async def fetch_customer(
    ...
    try:
        ...
        output_dto = await fetch_customer_use_case.execute(input_dto)
        return CustomerPresenter.present(output_dto)
        ...
        logger.exception("fetch_customer_failed")
        raise HTTPException(status_code=500)
...
async def update_customer(
    ...
    try:
        ...
        logger.exception("update_customer_failed")
        raise HTTPException(status_code=500)
...
async def delete_customer(
    ...
    try:
        ...
        logger.exception("delete_customer_failed")
        raise HTTPException(status_code=500)
